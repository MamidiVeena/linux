 ...
 * Here we used references from intel SDM to get pin based capabilities bits and description
 */
...
//Here we used references from intel SDM to get Procbased capabilities bits and description
...
//Here we used references from intel SDM to get secondary procbased capabilities bits and description
...
//Exit capabilities
//Here we used references from intel SDM to get exit capabilities bits and description
...
//Entry capabilities
//Here we used references from intel SDM to get entry based capabilities bits and description
...
void detect_vmx_features(void)
{
    ...
    //Checking for true controls support
    rdmsr(IA32_VMX_BASIC, lo, hi);
    if (IA32_VMX_SUPPORT_TRUE_CONTROLS_MASK & hi)
    {
        // Pinbased controls  MSR details to be shown in output
        printk(KERN_INFO "True Controls are Supported!");
        rdmsr(IA32_VMX_TRUE_PINBASED_CTLS, lo, hi);
        pr_info("The Pinbased Controls MSR details fetched are as follows: 0x%llx\n",
        ...
        //Procbased controls MSR details to be shown in output
        rdmsr(IA32_VMX_TRUE_PROCBASED_CTLS, lo, hi);
        pr_info("The Procbased Controls MSR details fetched are as follows: 0x%llx\n",
        (uint64_t)(lo | (uint64_t)hi << 32));
        ...
        if(IA32_VMX_ACTIVATE_SECONDARY_PROCBASED_CONTROLS_BIT_MASK & hi)
        {
        	...
        	// Secondary Procbased controls MSR details to be shown in output
        ...
        //Exit controls MSR details to be shown in output
        rdmsr(IA32_VMX_TRUE_EXIT_CTLS, lo, hi);
        pr_info("Exit Controls MSR details fetched: 0x%llx\n",
        (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(vm_exit, 11, lo, hi);
        pr_info("\n");

        //Entry controls MSR details to be shown in output
        rdmsr(IA32_VMX_TRUE_ENTRY_CTLS, lo, hi);
        pr_info("Entry Controls MSR details fetched: 0x%llx\n",
        (uint64_t)(lo | (uint64_t)hi << 32));
    ...
    else
    {
        ...
        pr_info("Pinbased Controls MSR details fetched: 0x%llx\n",
        (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(pinbased, 5, lo, hi);
		    pr_info("\n");
        ...
        pr_info("Procbased Controls MSR details fetched: 0x%llx\n",
        (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(procbased, 21, lo, hi);
        ...
        pr_info("Exit ControlsMSR details fetched: 0x%llx\n",
            (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(vm_exit, 11, lo, hi);
        pr_info("\n");
        ...
        pr_info("Entry Controls MSR details fetched are as follows: 0x%llx\n",
            (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(vm_entry, 9, lo, hi);
